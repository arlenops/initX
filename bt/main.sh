...
animate_welcome() {
  ...
  print_box_line "宝塔面板助手"
  print_box_line "覆盖安装与数据迁移场景"
  printf '%s+----------------------------------------+%s\n' "${COLOR_PRIMARY}" "${COLOR_RESET}"

  printf '\n'
  slow_print "${COLOR_SECONDARY}正在准备宝塔控制台 " 0.01
  local frames=('-' '\\' '|' '/')
  local frame
  for ((i = 0; i < 16; i++)); do
    frame=${frames[i % ${#frames[@]}]}
    printf '\r%s检查环境 %s' "${COLOR_SECONDARY}" "${frame}"
    sleep 0.08
  done
...
run_feature() {
  ...
  prompt_to_continue $'\033[38;5;111m按下回车返回宝塔菜单...\033[0m'
}
...
dispatch_menu_option() {
  ...
  case "${choice}" in
    1)
      run_feature "bt_install_fresh"
      ;;
    2)
      run_feature "bt_migrate_to_data_disk"
      ;;
    3)
      printf '%s即将返回。%s\n' "${COLOR_SECONDARY}" "${COLOR_RESET}"
      exit 0
