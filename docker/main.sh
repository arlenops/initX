...
animate_welcome() {
  ...
  print_box_line "Docker 运维助手"
  print_box_line "加速与数据盘迁移工具"
  printf '%s+----------------------------------------+%s\n' "${COLOR_PRIMARY}" "${COLOR_RESET}"

  printf '\n'
  slow_print "${COLOR_SECONDARY}正在加载 Docker 控制台 " 0.01
  local frames=('-' '\\' '|' '/')
  local frame
  for ((i = 0; i < 16; i++)); do
    frame=${frames[i % ${#frames[@]}]}
    printf '\r%s检测环境 %s' "${COLOR_SECONDARY}" "${frame}"
    sleep 0.08
  done
...
run_feature() {
  ...
  prompt_to_continue $'\033[38;5;111m按下回车返回 Docker 菜单...\033[0m'
}
...
dispatch_menu_option() {
  ...
  case "${choice}" in
    1)
      run_feature "docker_switch_accelerator"
      ;;
    2)
      run_feature "docker_migrate_to_data_disk"
      ;;
    3)
      printf '%s即将返回。%s\n' "${COLOR_SECONDARY}" "${COLOR_RESET}"
      exit 0
